...
from networks.efficient_net_lrl import ConEfficientNet, LinearClassifier
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch + 1, config.epoch_num):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            # ====== Encode HAI VIEW đúng 1 lần ======
            z_q, h_q, x_hat_q, p_q = model.forward_lrl(image1) 
            z_k, h_k, x_hat_k, p_k = model.forward_lrl(image2)

            # Tạo tensor [B,2,D] cho SupCon
            feats = torch.stack([z_q, z_k], dim=1)               # [B,2,128]

            # ====== Universum negatives (ảnh -> feature, không gradient) ======
            universum_imgs = get_universum_etd(image1, labels, config)   # [B,1,64,64]
            with torch.no_grad():
                uni_features = model(universum_imgs)                     # [B,128] (đã normalize trong model)
                # nếu model.forward đã normalize, UniConLoss(l2norm=True) vẫn OK

            # ====== Losses ======
            # SupCon + Universum
            L_uni = con_loss(features=feats, labels=labels, universum=uni_features)

            # Reconstruction (dùng cả hai view)
            L_rec = 0.0
            if x_hat_q is not None and x_hat_k is not None:
                L_rec = recon_loss(x_hat_q, image1) + recon_loss(x_hat_k, image2)

            # VICReg (song song q/k)
            L_vic = 0.0
            if (p_q is not None) and (p_k is not None):
                L_vic = vicreg_loss(p_q, p_k)

            alpha = 0.2   # weight recon
            beta  = 0.5   # weight VICReg
            loss = L_uni + alpha * L_rec + beta * L_vic
            ...
            if i % 100 == 0:
                ...
                with torch.no_grad():
                    sim_pos = (z_q * z_k).sum(1).mean()     # cos-pos (do đã normalize)
                    # neg: roll 1
                    z_neg = torch.roll(z_q, shifts=1, dims=0)
                    sim_neg = (z_q * z_neg).sum(1).mean()
                    sim_uni = (z_q * uni_features).sum(1).mean()
                    print(f'[sim] pos={sim_pos:.3f} neg={sim_neg:.3f} uni={sim_uni:.3f}')
                sys.stdout.flush()
