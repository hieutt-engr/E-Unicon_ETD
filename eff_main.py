...
def set_loader(opt):
    ...
    train_dataset = CANDataset(root_dir=opt.data_folder, window_size=32, is_train=True, transform=transform)
...
def train(train_loader, model, criterion, optimizer, epoch, opt, step):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        # compute logits & loss
        outputs = model(images)                    # logits from model
...
def validate(val_loader, model, criterion, opt):
    ...
    with torch.no_grad():
        for images, labels in tqdm(val_loader): 
            ...
            outputs = model(images)  # Trực tiếp ra logits

            bsz = labels.size(0)
            loss = criterion(outputs, labels)
            losses.update(loss.item(), bsz)
            
            pred = get_predict(outputs)  # Hàm này có thể là argmax(outputs, dim=1)
...
def main():
    ...
    for epoch in range(start_epoch, opt.epochs + 1):
        ...
        loss, val_acc, val_f1, precision, recall, conf_matrix = validate(test_loader, model, criterion, opt)
        print('Validation: Loss: {:.4f}, Acc: {}'.format(loss, val_acc))
